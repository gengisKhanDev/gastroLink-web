...
Template.admin_system_settings.events({
  "submit #companyInfo"(event){
    ...
    Meteor.call("company.info", name, address, phoneNumber, email, taxID,
  ...
  "submit #socialMedia"(event){
    ...
    Meteor.call("company.socialMedia", facebook, twitter, google, instagram,
      tiktok, function(error, result){
      if(error){
        console.log(error);
        yoloAlert("error");
  ...
  "submit #openingClosingTimes"(event){
    ...
    Meteor.call("openingClosingTimes", openingTimeMondayToThursday, closingTimeMondayToThursday,
      openingTimeFridaySaturday, closingTimeFridaySaturday, openingTimeSunday,
      closingTimeSunday, function(error, result){
      if(error){
        console.log(error);
        yoloAlert("error");
        disableBtn('#openingClosingTimes[type="submit"]', false, `<i class="fas fa-edit"></i> Update`);
      }
      else {
        setTimeout(function(){
          yoloAlert("success", "Updated Company Info!");
